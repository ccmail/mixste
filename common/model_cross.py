...
class CBlock(nn.Module):
    ...
    def __init__(
        ...
        self.pos_embed = nn.Conv1d(in_channels=dim, out_channels=dim, kernel_size=3, padding=1, groups=1)
        # self.norm1 = nn.BatchNorm2d(dim)
        self.norm1 = nn.BatchNorm1d(dim)
        # self.conv1 = nn.Conv2d(dim, dim, 1)
        self.conv1 = nn.Conv1d(in_channels=dim, out_channels=dim, kernel_size=1)
        # self.conv2 = nn.Conv2d(dim, dim, 1)
        self.conv2 = nn.Conv1d(in_channels=dim, out_channels=dim, kernel_size=1)
        # self.attn = nn.Conv2d(dim, dim, 5, padding=2, groups=dim)
        self.attn = nn.Conv1d(in_channels=dim, out_channels=dim, kernel_size=5, padding=2, groups=dim)
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
...
class MixSTE2(nn.Module):
    def __init__(self, num_frame=9, num_joints=17, in_chans=2, embed_dim_ratio=32, depth=4,
        ...
        self.ste_blocks = nn.ModuleList([
            CBlock(
                ...
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer) if i < depth / 2 else
            SABlock(
                dim=embed_dim_ratio, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer)
            for i in range(depth)])

        self.tte_blocks = nn.ModuleList([
            CBlock(
                dim=embed_dim_ratio, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer) if i < depth / 2 else
            SABlock(
    ...
    def st_forward(self, x):
        ...
        for i in range(1, self.block_depth):
            ...
            steblock = self.ste_blocks[i]
            tteblock = self.tte_blocks[i]
